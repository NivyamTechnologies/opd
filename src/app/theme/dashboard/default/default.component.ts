...
monthPatient:any;
  ...
  ngOnInit() {
debugger
    let qry = "SELECT sum(TotalAmount) total from t_sale_master where DocNo=27 and YEAR(CreatedDate) = YEAR(CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry}).subscribe(item=>{
     this.totalodp = item["data"][0].total
    
    })
    let qry1 = "SELECT ifnull(sum(TotalAmount),0) totalmonth from t_sale_master where DocNo=27 and  month(CreatedDate) = month(CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry1}).subscribe(item=>{
     this.monthodp = item["data"][0].totalmonth
    
    })
    let qry2 = "SELECT sum(TotalAmount) total from t_sale_master where DocNo=28 and YEAR(CreatedDate) = YEAR(CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry2}).subscribe(item=>{
     this.totalidp = item["data"][0].total
    
    }) 
 

    let qry3 = "SELECT ifnull(sum(TotalAmount),0) totalmonth from t_sale_master where DocNo=28 and  month(CreatedDate) = month(CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry3}).subscribe(item=>{
     this.monthidp = item["data"][0].totalmonth
    
    })
   
    
    let qry4 = "SELECT count(DISTINCT PatientMobile) Patient FROM t_sale_master where  month(CreatedDate) = month(CURDATE())"
    this.api.Post("/users/executeSelectStatement",{Query : qry4}).subscribe(item=>{
     this.monthPatient = item["data"][0].Patient
    
    })

        
    let qry5 = "SELECT count(DISTINCT PatientMobile) Patient FROM t_sale_master WHERE  CreatedDate > CURDATE()- INTERVAL 7 day"
    this.api.Post("/users/executeSelectStatement",{Query : qry5}).subscribe(item=>{
     this.last7Patient = item["data"][0].Patient
