...
company:any;
  ...
  {  if (!this.api.netConnectivity()) {
    ...
    let qry = "Select ItemId,ItemName,Qty,rate from item where Active='Y' and type='Opd'"
 ...
 applydiscount()
 {
   ...
   this.model['discount']= (this.totaldiscount).toString();
    this.processSale.insertintoSaleMaster(this.model).subscribe(SaleId=>{
      this.processSale.insertintoSaleDetail(this.dataRows,SaleId).subscribe(data=>{
        ...
        console.log("Error while inserting into Odp detail")
      })
    },err=>{
      console.log("Error while inserting into Odp Master",err)
    })
  ...
  printInvoice() // function to generate invoice data
  {
    if (!this.api.netConnectivity()) {
      ...
      localStorage.setItem("invoice",JSON.stringify({"form":this.model,"table":this.dataRows,"state":"OPD"})) // storing data to print on invoice
