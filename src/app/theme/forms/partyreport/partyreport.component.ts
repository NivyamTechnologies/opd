...
export class PartyreportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sql1('select * from doctor').subscribe(res=>{
  ...
  editparty(row)
  {
    this.route.navigate(['forms/party',{'doctorId' : row['doctorId']}])
  }
  
  deleteparty(doctorId,rowIndex)
  {
    let qry = "Delete from doctor where doctorId = "+doctorId
